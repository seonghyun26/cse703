...
def load(args):
    ...
    if args["data_name"].startswith('ogbl'):
        ...
        # train, val, test mask -> 140, 500, 1000 out of 2708
        data.edge_index = to_undirected(data.edge_index)
        if args["data_name"] in ["Cora", "Citeseer"]:
            neg_pool_max = True
        else:
            neg_pool_max = False
    elif args["data_name"].startswith("jssp"):
        # if args["seed"] >= 0:
        #     data = torch.load("./dataset/jssp/jssp_example_graph2.pt")
        # else:
        data = torch.load("./dataset/jssp/jssp_" + args["instanceSize"] + "_" +str(args["seed"])+".pt")
        neg_pool_max = True
    else:
...
def do_edge_split(data, val_ratio=0.05, test_ratio=0.1, neg_pool_max = False):
    ...
    # print(data)
    # Data(
        # x=[2708, 1433], y=[2708], train_mask=[2708], val_mask=[2708], test_mask=[2708],
        # val_pos_edge_index=[2, 263], test_pos_edge_index=[2, 527], train_pos_edge_index=[2, 4488],
        # train_neg_adj_mask=[2708, 2708], val_neg_edge_index=[2, 263], test_neg_edge_index=[2, 527]
    # )
